...
void philosopher(int n, std::mutex *left, std::mutex *right)
{
    while (true)
    {
        ...
        out.unlock();
        // Attempt to lock the right fork using try_lock
        if (right->try_lock())
        {
            out.lock();
            ...
            right->unlock(); // Release the right fork
        }
        else
        {
            out.lock();
            std::cout << "Philosopher " << n << " couldn't pick up her right fork and put down her left fork." << std::endl;
            out.unlock();

            left->unlock(); // Release the left fork since the right fork couldn't be picked up
        }
        out.lock();
